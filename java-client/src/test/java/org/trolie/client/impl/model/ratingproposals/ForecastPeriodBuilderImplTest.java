...
import org.trolie.client.model.common.RatingUnits;
...
class ForecastPeriodBuilderImplTest {
    ...
    void complete_typical_success() {
        ...
        assertEquals(700f, period.getContinuousOperatingLimit()
                .getMVA());
        var emergRatings = period.getEmergencyOperatingLimits();
        assertNotNull(emergRatings);
        assertEquals(2, emergRatings.size());
        assertEquals(750f, emergRatings.get(0).getLimit().getMVA());
        assertEquals(BAND1, emergRatings.get(0).getDurationName());
        assertEquals(780f, emergRatings.get(1).getLimit().getMVA());
        assertEquals(BAND2, emergRatings.get(1).getDurationName());
    }
    ...
    void amps_units() {
        ...
        var period = periodCaptor.getValue();
        assertNotNull(period);
        assertEquals(500f, period.getContinuousOperatingLimit()
                .getAMPS());
        var emergRatings = period.getEmergencyOperatingLimits();
        assertNotNull(emergRatings);
        assertEquals(2, emergRatings.size());
        assertEquals(550f, emergRatings.get(0).getLimit().getAMPS());
        assertEquals(580f, emergRatings.get(1).getLimit().getAMPS());
    }
    ...
    void mwpf_units() {
        ...
        assertEquals(500f, continuous.getLeft());
        assertEquals(0.9f, continuous.getRight());
        var emergRatings = period.getEmergencyOperatingLimits();
        assertNotNull(emergRatings);
        assertEquals(2, emergRatings.size());
        var emer1 = emergRatings.get(0).getLimit().getMWAndPF();
        assertEquals(550f, emer1.getLeft());
        assertEquals(0.8f, emer1.getRight());
        var emer2 = emergRatings.get(1).getLimit().getMWAndPF();
        assertEquals(580f, emer2.getLeft());
        assertEquals(0.8f, emer2.getRight());
    }
